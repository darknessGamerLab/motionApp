# Spotlights / motionApp — Kıdemli İnceleme Özeti (v5.0.3)
...
**v5.0.3:** `web/src/lib/api.ts` içinde `axios.create({ baseURL })` ile ortak istemci; admin girişi `X-Admin-Key` = API’deki `ADMIN_API_KEY` ile aynı olmalı. `app.json`: `android.versionCode` / `ios.buildNumber` EAS/prebuild ile mağaza sürümü için senkron.
...
*Sürüm: **5.0.3** — `package.json` ve `app.json` ile uyumludur.*
